        ...
        while(ros::ok())
        {
            try
            {
                ...
                else
                {
                    ...
                    cv::Mat dst;
                    dst = img.clone();
                    cv::GaussianBlur(img, dst, cv::Size(5,5), 2, 2);// sigmax, sigmay
                    cv::resize(dst, img, cv::Size(dst.cols*0.5,dst.rows*0.5), 0, 0, CV_INTER_LINEAR);//resize to 1024x1024
...
int main(int argc, char* argv[]) {
    ...
    publisher[0]= itl.advertise("imager", 1);
    publisher[1] = itr.advertise("imagel", 1);
